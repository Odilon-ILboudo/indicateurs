...
export class CoursePresenter implements OnDestroy {
  ...
  async courseLeaderboard(): Promise<CourseLeaderboardEntry[]> {
    ...
    return firstValueFrom(this.resultService.courseLeaderboard({ courseId: course.id }))
  }

  async activityLeaderboard(activityId: string): Promise<ActivityLeaderboardEntry[]> {
    return firstValueFrom(this.resultService.activityLeaderboard({ activityId }))
  }
        ...
        courseGroups: this.context.value.courseGroups
          ?.map((group) => {
            if (group.courseGroup.groupId === groupId) {
              return { ...group, courseGroup: { ...group.courseGroup, name: newName } }
            }
            return group
          })
  ...
  async listCourseGroupMembers(groupId: string): Promise<CourseMember[]> {
    ...
    this.context.next({
      ...
      courseGroups: this.context.value.courseGroups?.map((group) => {
        if (group.courseGroup.groupId === groupId) {
          return { ...group, members: response.resources }
        }
        return group
      }),
    })
  ...
  async removeGroupMember(groupId: string, member: CourseMember): Promise<void> {
    ...
    try {
      ...
      this.context.next({
        ...
        courseGroups: this.context.value.courseGroups?.map((group) => {
          if (group.courseGroup.groupId === groupId) {
            return { ...group, members: group.members.filter((m) => m.user!.id !== member.user!.id) }
          }
          return group
        }),
      })
  ...
  async addGroupMember(groupId: string, members: CourseMember[]): Promise<void> {
    ...
    try {
      ...
      this.context.next({
        ...
        courseGroups: this.context.value.courseGroups?.map((group) => {
          if (group.courseGroup.groupId === groupId) {
            return { ...group, members: group.members.concat(members.map((m) => ({ ...m, createdAt: new Date() }))) }
          }
          return group
        }),
      })
  ...
  async addGroup(): Promise<void> {
    ...
    try {
      ...
      this.context.next({
        ...this.context.value,
        courseGroups: this.context.value.courseGroups?.concat({ courseGroup: group.resource, members: [] }),
      })
    } catch {
      this.alertError()
    }
  ...
  async deleteGroup(groupId: string): Promise<void> {
    ...
    try {
      ...
      this.context.next({
        ...this.context.value,
        courseGroups: this.context.value.courseGroups?.filter(
          (group) => group.courseGroup.groupId !== groupId
        ),
      })
    } catch {
      this.alertError()
