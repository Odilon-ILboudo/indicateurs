...
@Component({
  ...
  template: `
    <div class="search-bar">
      <input
        class="search-input"
        [placeholder]="searchbar?.placeholder || 'Rechercher...'"
        [formControl]="control"
        (keyup.enter)="onTrigger()"
      />
      <button class="filter-btn" *ngIf="filterVisible" (click)="onFilter()">
  ...
  styles: [`
    ...
    .search-input {
      ...
      font-size: 0.95rem;
      color: var(--brand-text-primary, #333);
    }
    .filter-btn {
      background: none;
      border: none;
...
export class UiSearchBarComponent implements OnInit, OnDestroy {
  private readonly subscriptions: Subscription[] = []

  @Input() searchbar?: SearchBar<unknown>
  ...
  ngOnInit(): void {
    this.filterVisible = !!this.searchbar?.onFilter || this.filter.observed
    ...
    setTimeout(() => this.searchbar?.onReady?.())
  }
...
@Component({
  ...
  template: `
    <mat-chip-set *ngIf="!empty">
      @for (match of matches; track $index) {
        <mat-chip [removable]="true" (removed)="onRemoveIndicator(match.indicator)">
          {{ match.label }}
...
export class UiFilterIndicatorComponent<T = unknown> implements OnChanges {
  private readonly cdr = inject(ChangeDetectorRef)

  protected empty = true
  protected matches: { label: string; indicator: FilterIndicator<T> }[] = []
  ...
  async ngOnChanges(): Promise<void> {
    const { filters } = this
    if (!filters) { this.empty = true; this.matches = []; return }
    this.matches = await Promise.all(
      this.indicators
        .filter((i) => i.match(filters))
        .map(async (i) => ({ label: await i.describe(filters), indicator: i }))
    )
